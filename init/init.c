...
/*   Updated: 2023/12/19 18:40:53 by abuonomo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (cub3d->map[i])
	{
		while (cub3d->map[i][j])
		{
			if (cub3d->map[i][j] == '2')
			{
				...
				cub3d->sprite[k].distance = ((cub3d->player.posX - cub3d->sprite[k].x) * (cub3d->player.posX - cub3d->sprite[k].x) + (cub3d->player.posY - cub3d->sprite[k].y) * (cub3d->player.posX - cub3d->sprite[k].y));
				printf("sprite[%d].distance = %f\n", k, cub3d->sprite[k].distance);
				k++;
			}
			j++;
		}
